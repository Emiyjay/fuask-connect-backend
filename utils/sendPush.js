const { messaging, initialized } = require('../config/firebase')

async function sendPushNotification(fcmToken, title, body, data = {}) {
  if (!initialized || !fcmToken) return null
  try {
    return await messaging.send({ token: fcmToken, notification: { title, body }, data })
  } catch (error) {
...
async function sendPushToMany(fcmTokens, title, body, data = {}) {
  ...
  try {
    return await messaging.sendEachForMulticast({ tokens: validTokens, notification: { title, body }, data })
  } catch (error) {
